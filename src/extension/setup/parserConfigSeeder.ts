...
export async function ensureParserConfigExists(
  ...
  try { await vscode.workspace.fs.stat(cfgUri); cfgExists = true; } catch {}
  try { await vscode.workspace.fs.stat(readmeUri); readmeExists = true; } catch {}
...
export async function migrateParserConfigIfNeeded(
  ...
  let newExists = false;
  try { await vscode.workspace.fs.stat(newDir); newExists = true; } catch {}

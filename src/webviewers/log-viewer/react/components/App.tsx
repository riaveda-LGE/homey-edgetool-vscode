...
export function App() {
  ...
  const ui = createUiLog(vscode, 'log-viewer.app');
  return (
    <div
      className="tw-h-full tw-grid tw-min-h-0"
      style={{ gridTemplateRows: '44px minmax(0,1fr) auto' }}
    >
      ...
      <div
        className={`tw-grid tw-min-h-0 ${showBookmarks ? 'tw-grid-cols-[1fr_6px_260px]' : 'tw-grid-cols-1'}`}
      >
        ...
        {showBookmarks && (
          <div className="tw-bg-[var(--panel)] tw-border-x tw-border-[var(--border)]" />
        )}
